...
public class VoidJadeOre extends BlockBase {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		if(dropChance <= 50) {
			return Item.getItemFromBlock(JadeRegistration.void_jade_raw);
		}else if(dropChance <= 85) {
			return Item.getItemFromBlock(JadeRegistration.void_jade_raw1);
		}else{
			return Item.getItemFromBlock(JadeRegistration.void_jade_raw2);
		}
	}
}
