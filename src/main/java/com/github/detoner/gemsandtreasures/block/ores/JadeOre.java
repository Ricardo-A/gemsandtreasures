...
public class JadeOre extends BlockBase {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		if(dropChance <= 40) {
			return Item.getItemFromBlock(JadeRegistration.jade_raw);
		}else if(dropChance < 70) {
			return Item.getItemFromBlock(JadeRegistration.jade_raw1);
		}else if(dropChance < 90){
			return Item.getItemFromBlock(JadeRegistration.jade_raw2);
		}else if(dropChance < 99){
			return Item.getItemFromBlock(JadeRegistration.jade_raw_white);
		}else{
			return Item.getItemFromBlock(JadeRegistration.jade_raw_golden);
		}
	}
}
